#include"Ball.h"
Ball::Ball():Object(),radius(12.5f),isStuck(GL_TRUE)	//用于子类的初始化
{

}
Ball::Ball(glm::vec2 pos, GLfloat radius, glm::vec2 velocity, Texture2D sprite):	//双构造
Object(pos, glm::vec2(radius * 2, radius * 2), sprite, glm::vec3(1.0f), velocity), isStuck(GL_TRUE), radius(radius)
{
...
glm::vec2 Ball::move(GLfloat dt, GLuint window_width,glm::vec2 pos)
{
	...
	{
		this->pos += this->velocity * dt;	//与键盘输入不同
		if (this->pos.x <= 0.0f)
		{
			this->pos.x = 0.0f;
